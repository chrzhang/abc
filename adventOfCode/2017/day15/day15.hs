...
{-
From http://adventofcode.com/2017/day/15
...
getlower16bits :: Int -> Int
getlower16bits n = n .&. 0xFFFF
...
day15a_solve :: Int -> Int -> Int
day15a_solve a_start b_start
    = foldr (\(x, y) z -> if lower16bitsmatch x y then z + 1 else z) 0 pairs
      where z_values = zip a_values b_values
            a_values = tail $ iterate (nextvalue a_factor) a_start
            b_values = tail $ iterate (nextvalue b_factor) b_start
            pairs = take 40000000 z_values

day15b_solve :: Int -> Int -> Int
day15b_solve a_start b_start
    = foldr (\(x, y) z -> if lower16bitsmatch x y then z + 1 else z) 0 pairs
      where z_values = zip a_values b_values
            a_values = tail $ iterate (nextvalue' a_factor 4) a_start
            b_values = tail $ iterate (nextvalue' b_factor 8) b_start
            pairs = take 5000000 z_values
            nextvalue' mf mu n = head $ filter (\x -> x `mod` mu == 0) $ tail $ iterate (nextvalue mf) n

main :: IO ()
main = do
    let day15a_result = day15a_solve 512 191
    let day15b_result = day15b_solve 512 191
    putStrLn $ unwords [ assert (588 == day15a_solve 65 8921) "+",
                         assert (309 == day15b_solve 65 8921) "+",
                         assert (567 == day15a_result) "+",
                         assert (323 == day15b_result) "+" ]
    putStrLn $ "Part 1: " ++ show day15a_result
    putStrLn $ "Part 2: " ++ show day15b_result
