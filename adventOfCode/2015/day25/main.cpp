...
/*
From https://adventofcode.com/2015/day/25
...
int toOrderedIndex(const int row, const int column) {
    // Given a row, column, return the index of the code in the order generated
    int index = 0;
...
void runTests() {
    assert(toOrderedIndex(3, 3) == 13);
    assert(toOrderedIndex(1, 1) == 1);
    assert(toOrderedIndex(1, 6) == 21);
    assert(toOrderedIndex(5, 2) == 17);
}

int main() {
    // Input is row 2978 column 3083
    const int index = toOrderedIndex(2978, 3083);
    long long int codeForMachine = 20151125; // First code
    for (int i = 0; i < index - 1; ++i) {
        codeForMachine = codeForMachine * 252533 % 33554393;
    }
    assert(codeForMachine == 2650453);
    std::cout << codeForMachine << std::endl;
    return 0;
}
