...
/*
From http://adventofcode.com/2015/day/20
...
/*
A house has been visited by elves e1, e2, ... en so it receives 10 * e1 +
10 * e2 + ... + 10 * en gifts or 10 * (e1 + e2 + ... + en) gifts. To find the
first house that receives 29 million gifts we can instead see which house
first receives 2.9 million gifts if each elf only gifts 1 * their present.
*/
int part1(const int threshold) {
    std::unordered_map<int, int> house_gifts;
    int result = INT_MAX;
    // threshold / 10 serves as a generous upper bound for the number of elves
    // since the (threshold / 10)th elf alone will deliver enough presents to
    // reach threshold
    for (int elf = 1; elf <= threshold / 10; ++elf) {
        // threshold / 10 also serves as a generous upper bound for the number
        // of houses since the (threshold / 10)th house will be visited by the
        // (threshold / 10)th elf
        for (int house = elf; house <= threshold / 10; house += elf) {
            house_gifts[house] += elf * 1; // 1 is gifts per elf
            if (house_gifts[house] >= threshold / 10) {
                result = std::min(result, house);
            }
        }
    }
    return result;
...
/*
Similar logic to part1()
*/
int part2(const int threshold) {
    std::unordered_map<int, int> house_gifts;
    int result = INT_MAX;
    for (int elf = 1; elf <= threshold / 11; ++elf) {
        for (int house = elf, iter = 1;
             house <= threshold / 11 &&
             iter <= 50; house += elf, ++iter) {
            house_gifts[house] += elf * 11;
            if (house_gifts[house] >= threshold) {
                result = std::min(result, house);
            }
        }
    }
...
int main() {
    std::cout << "Part 1: " << part1(29000000) << std::endl;
    std::cout << "Part 2: " << part2(29000000) << std::endl;
    return 0;
}
