...
class Disc {
    ...
    int d_numberOfPositions;
    int d_initialPosition;

    public:

        Disc(const int numberOfPositions, const int initialPosition)
        : d_numberOfPositions(numberOfPositions),
          d_initialPosition(initialPosition) {
        }

        const int & numberOfPositions() const {
            return d_numberOfPositions;
        }

        const int getPositionAt(const int time) const {
            return (d_initialPosition + time) % d_numberOfPositions;
        }

};

int solve(const std::vector<Disc> & discs) {
    for (int startTime = 0; ; ++startTime) {
        bool allDiscsAligned = true;
        for (size_t ii = 0; ii < discs.size(); ++ii) {
            if (discs[ii].getPositionAt(startTime + ii + 1) != 0) {
                allDiscsAligned = false;
                break;
            }
        }
        if (allDiscsAligned) {
