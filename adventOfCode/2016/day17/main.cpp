...
bool doorIsOpen(const char c) {
    return (c >= 'b') && (c <= 'f');
}

void exploreAux(Result & currentShortestResult,
                const std::string & currentInput,
                const int currentX,
                const int currentY,
                std::function<void(Result &, const std::string &, const int, const int)> explore) {
    char hashed[HASH_LEN] = { 0 };
    md5(currentInput.c_str(), hashed);
    if ((currentY > 0) && doorIsOpen(hashed[0])) { // Can go up
        explore(currentShortestResult, currentInput + 'U', currentX, currentY - 1);
    }
    if ((currentY < 3) && doorIsOpen(hashed[1])) { // Can go down
        explore(currentShortestResult, currentInput + 'D', currentX, currentY + 1);
    }
    if ((currentX > 0) && doorIsOpen(hashed[2])) { // Can go left
        explore(currentShortestResult, currentInput + 'L', currentX - 1, currentY);
    }
    if ((currentX < 3) && doorIsOpen(hashed[3])) { // Can go right
        explore(currentShortestResult, currentInput + 'R', currentX + 1, currentY);
    }
}

void exploreShortestPath(Result & currentShortestResult,
                         const std::string & currentInput,
                         const int currentX,
                         const int currentY) {
    if (currentShortestResult.wasSet() &&
        (currentInput.size() >= currentShortestResult.getLength())) {
        return;
    }
    if (currentX == 3 && currentY == 3) {
        currentShortestResult.set(currentInput);
        return;
    }
    exploreAux(currentShortestResult, currentInput, currentX, currentY,
               exploreShortestPath);
}

void exploreLongestPath(Result & currentLongestResult,
             const std::string & currentInput,
             const int currentX,
             const int currentY) {
    if (currentX == 3 && currentY == 3) {
        if ((currentLongestResult.wasSet() &&
            (currentLongestResult.getLength() < currentInput.size())) ||
            (!currentLongestResult.wasSet())) {
            currentLongestResult.set(currentInput);
        }
        return;
    }
    exploreAux(currentLongestResult, currentInput, currentX, currentY,
               exploreLongestPath);
}

const std::string solveShortestPath(const std::string & input) {
    Result result;
    exploreShortestPath(result, input, 0, 0);
    return std::string(result.getResult().begin() + input.size(),
                       result.getResult().end());
}

const std::string solveLongestPath(const std::string & input) {
    Result result;
    exploreLongestPath(result, input, 0, 0);
    return std::string(result.getResult().begin() + input.size(),
                       result.getResult().end());
}

void test() {
    assert(solveShortestPath("ihgpwlah") == "DDRRRD");
    assert(solveShortestPath("kglvqrro") == "DDUDRLRRUDRD");
    assert(solveShortestPath("ulqzkmiv") == "DRURDRUDDLLDLUURRDULRLDUUDDDRR");
}

int main() {
    test();
    {
        const auto part1Result = solveShortestPath("lpvhkcbi");
        assert("DUDRLRRDDR" == part1Result);
        std::cout << "Part 1: " << part1Result << std::endl;
    }
    {
        const auto part2Result = solveLongestPath("lpvhkcbi").size();
        assert(788 == part2Result);
